...
description: Step-by-step guide to run the full log0 pipeline on your machine - Docker infrastructure, environment variables, all five services, health checks, log submission, and Kafka/PostgreSQL inspection.
---
...
The **notification-service** requires Slack credentials at startup. The other four services need no additional configuration.
...
| normalization-service | 8081 | Parses and fingerprints logs |
...
A full end-to-end test (10 logs → Slack message) takes about 2–3 seconds under normal load.
