...
stateDiagram-v2
    ...
    NEW --> RESOLVED : Auto-resolve if no new logs (optional)
...
sequenceDiagram
    ...
    participant AIS as AI Summary Service (8085)
    participant Cache as Redis (future)
    participant CH as ClickHouse (future)
    participant LLM as LLM Provider (Strategy)
    participant PG as PostgreSQL

    IS->>AIS: POST /api/v1/summaries (async - does not block notification)
    Note over AIS: Request body: incidentId · tenantId · serviceName · environment<br/>severity · occurrenceCount · firstSeenAt · lastSeenAt · topMessages

    AIS->>Cache: GET summary:incident:incidentId [future - Phase 9]
    Cache-->>AIS: MISS

    AIS->>AIS: PromptBuilder.build(request) - structured prompt
    Note over AIS: MVP uses topMessages from request<br/>Future (Phase 4.5): enrich with ClickHouse query below

    AIS->>CH: Query top 10 messages by fingerprint and tenant [future - Phase 4.5]
    CH-->>AIS: top 10 most frequent messages with occurrence counts

    AIS->>LLM: provider.generateSummary(prompt) temperature=0.2 maxTokens=300
    Note over LLM: Active provider selected by ai.provider config<br/>Groq (default) · OpenAI · Gemini · Anthropic<br/>All implement LlmProvider interface (Strategy Pattern)
    LLM-->>AIS: AI-generated summary text

    AIS->>Cache: SET summary:incident:incidentId TTL=1h [future - Phase 9]
    AIS->>IS: PATCH /api/v1/incidents/incidentId/ai-summary
    IS->>PG: UPDATE incidents SET ai_summary=...

    Note over IS: ai_summary stored - next notification will include it
```
...
sequenceDiagram
    ...
    AS->>PG: SELECT * FROM user WHERE email = ?
    PG-->>AS: User row (email, passwordHash, role, tenantId)
    AS->>AS: BCrypt.matches(password, passwordHash)

    alt invalid credentials
        AS-->>FE: 401 Unauthorized
    else valid
        AS->>AS: JwtUtil.generateToken(userId, tenantId, role) → accessToken (1h)
        AS->>AS: generate refreshToken → SHA-256(refreshToken) → store in DB (30d)
        AS-->>FE: 200 OK { accessToken, refreshToken, expiresIn: 3600 }
    end
```
...
sequenceDiagram
    ...
    FE->>AS: POST /api/v1/auth/refresh { refreshToken }
    AS->>AS: SHA-256(refreshToken) → look up in refresh_token table
    AS->>PG: SELECT * FROM refresh_token WHERE token_hash = ? AND revoked = false
    PG-->>AS: token row

    alt expired or revoked
        AS-->>FE: 401 Unauthorized
    else valid
        AS->>PG: UPDATE refresh_token SET revoked = true (rotate old token)
        AS->>AS: generate new refreshToken + new accessToken
        AS->>PG: INSERT new refresh_token row
        AS-->>FE: 200 OK { accessToken, refreshToken, expiresIn: 3600 }
    end
```
...
sequenceDiagram
    participant FE as Frontend (3000)
    participant IS as Incident Service (8083)
    participant SC as SecurityContext

    FE->>IS: GET /api/v1/incidents?tenantId=xyz<br/>Authorization: Bearer eyJhbGci...

    IS->>IS: JwtAuthFilter.doFilterInternal()
    IS->>IS: JwtUtil.validateToken(token)<br/>- verify HS256 signature with JWT_SECRET<br/>- check exp claim not expired

    alt invalid or expired token
        IS-->>FE: 401 Unauthorized
    else valid
        IS->>IS: JwtUtil.extractClaims() → userId, tenantId, role
        IS->>SC: SecurityContextHolder.setAuthentication(...)
        IS->>IS: IncidentController.listIncidents(tenantId, pageable)
        IS-->>FE: 200 OK [ ...incidents ]
    end
```
...
```mermaid
sequenceDiagram
    participant SDK as Customer Service (SDK)
    participant IG as Ingestion Gateway (8080)
    participant AS as Auth Service (8086)
    participant PG as PostgreSQL

    SDK->>IG: POST /api/v1/logs<br/>X-TENANT-ID: uuid<br/>X-API-KEY: ak_rawkey123

    IG->>AS: POST /api/v1/auth/validate-key<br/>{ tenantId, apiKey: ak_rawkey123 }
    AS->>AS: SHA-256(ak_rawkey123) → keyHash
    AS->>PG: SELECT * FROM api_key<br/>WHERE tenant_id = ? AND key_hash = ?<br/>AND is_active = true<br/>AND (expires_at IS NULL OR expires_at > NOW())

    alt key not found or inactive
        AS-->>IG: 401 Unauthorized
        IG-->>SDK: 401 Unauthorized
    else valid key
        AS-->>IG: 200 OK
        IG->>IG: publish to Kafka raw-logs topic
        IG-->>SDK: 202 Accepted
    end
```
...
sequenceDiagram
    ...
    FE->>AS: POST /api/v1/auth/api-keys<br/>Authorization: Bearer adminJwt<br/>{ name: "production-key" }
    AS->>AS: generate rawKey (UUID-based, high entropy)
    AS->>AS: keyHash = SHA-256(rawKey)
    AS->>PG: INSERT INTO api_key (tenant_id, key_hash, name, is_active)
    AS-->>FE: 201 Created { keyId, rawKey, name, createdAt }<br/>⚠ rawKey shown ONCE - not stored, not retrievable
```
