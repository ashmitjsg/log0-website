...
The contracts are maintained as an [AsyncAPI](https://www.asyncapi.com/) specification - the event-driven equivalent of OpenAPI/Swagger - in `docs/asyncapi/asyncapi.yaml` in the `log0-platform` repository.
...
The source file lives at `docs/asyncapi/asyncapi.yaml` in the `log0-platform` repository.
...
Run this from the root of the `log0-platform` repository:
